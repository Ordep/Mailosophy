...
def get_imap_configuration() -> Dict[str, Optional[str]]:
    data = get_config_values(['imap_email', 'imap_password', 'imap_server'])
    return {
        'email': (data.get('imap_email') or '').strip() or None,
        'password': (data.get('imap_password') or '').strip() or None,
        'server': (data.get('imap_server') or '').strip() or None,
    }
